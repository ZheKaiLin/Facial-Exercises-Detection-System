...
比較 YOLOv7-tiny 與 YOLOv9-tiny 於測試集之辨識效能，以及兩模型於相同條件下之端到端效能，決定最終整合進系統的模型。
...
YOLOv9-tiny 整體 F1-score、mAP@0.5 均略優於 YOLOv7-tiny。

**端到端處理延遲**（表 20，逐幀量測，i5-12600KF）：
...
不含攝影機擷取與 GUI 繪製，純粹是模型 + MediaPipe 的運算延遲；v7 全程快於 v9。

**實際顯示吞吐量**（表 21，直接執行正式系統程式量測，含攝影機擷取＋推論＋GUI 繪製）：

| 量測項目 | 模型 | 平均FPS | SD(FPS) | P5(FPS) |
|---|---|---|---|---|
| i5-12600KF | v7 | 24.17 | 0.95 | 22.83 |
| i5-12600KF | v9 | 22.61 | 0.87 | 21.73 |
| i7-8565U（目標裝置，行動省電處理器，TDP 15W） | v7 | 9.89 | 1.54 | 9.24 |
| i7-8565U（目標裝置，行動省電處理器，TDP 15W） | v9 | 9.46 | 0.52 | 8.67 |

含 GUI 之實際 FPS 遠低於表 20 之處理延遲換算產能，主要成本來自攝影機影像前處理與畫面合成繪製（非模型推論本身）；目標裝置 i7-8565U 上兩模型 FPS 已無實質差異（9.89 對 9.46）。

**結論**：表 20 的純延遲層次 v7 較快，但在含 GUI 的實際使用情境下兩模型 FPS 差距已不具意義，因此改以辨識效能為優先，**最終採用 YOLOv9-tiny（b32 e225）**。細節與統計檢定見論文 3.4、5.3 節。

### 常見問題

**資料建置的三個步驟都有程式碼嗎？**
分層抽樣（`01_train_val_split/train_val_split.py`，固定 `random_state=42`）與少數類別擴增（`02_balance_augmentation/make_5_balanced_data.py`）都有。外部圖庫測試集（test_100）只有 Google Drive 圖片連結、沒有蒐集程式碼，但標註品質查核有（`03_test_100_annotation_reliability/`，雙人盲標註 kappa/IoU）。

**YOLOv7/v9 的訓練語法、batch size 選權重、匯出 ONNX 呢？**
訓練/驗證/匯出指令都在 `model_training/yolov7(9)/commands.txt`。挑選最佳權重不需要額外程式碼，YOLO 訓練框架本身會依驗證集 mAP 自動存下最佳 epoch（`best.pt`）。
