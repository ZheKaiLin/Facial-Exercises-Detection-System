...
SRC_ROOT = Path("2 v4_data_train_val_split")              # 原始資料集資料夾
...
def copy_dataset_structure():
    if not SRC_ROOT.exists():
        raise FileNotFoundError(f"找不到來源資料集 -> {SRC_ROOT}（請先執行 01_train_val_split/train_val_split.py，或修改本檔開頭的 SRC_ROOT）")
